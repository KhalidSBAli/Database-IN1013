...
FROM Waiters w JOIN Bills b ON w.waiter_id = b.waiter_id
...
FROM Rooms r JOIN Tables t ON r.room_id = t.room_id
WHERE t.capacity > 6 GROUP BY r.room_name;


SELECT r.room_name, 
       SUM(b.total_amount) AS total_bills
FROM Rooms r JOIN Bills b ON r.room_id = b.room_id
...
GROUP BY h.name, h.surname ORDER BY total_bill_amount DESC;


SELECT c.name AS customer_name FROM Customers c
JOIN Bills b ON c.customer_id = b.customer_id
GROUP BY c.name HAVING AVG(b.total_amount) > 400.00;
